pub mod day_09 {
    ...
    fn parse(filename: &str) -> Result<Vec<char>, Box<dyn Error>> {
        ...
        Ok(content
            ...
            .map(|l| {
                let mut x = l.split(' ');
                (x.next().expect("something").parse::<char>().expect("a char"), x.next().expect("something").parse::<usize>().expect("a number"))
            })
    ...
    #[allow(dead_code)]
    fn output(historyp1: &HashSet<(i32,i32)>,historyp2: &HashSet<(i32,i32)>,points:&Vec<(i32,i32)>) {
        ...
        let mut grid = vec![vec!['.';(maxy.1+miny.1.abs()+3) as usize];(maxx.0+minx.0.abs()+3) as usize];
        ...
        points.iter().for_each(|tail| grid[(tail.0 + minx.0.abs()) as usize][(tail.1 + miny.1.abs()) as usize] = 'H');
    ...
    pub fn solve(filename: &str) -> Result<(usize, usize), Box<dyn Error>> {
        ...
        data.iter().for_each(|c| {
            ...
            for idx in 1..points.len(){
                if ((points[idx-1].0 - points[idx].0) as i32).abs() < 2 && ((points[idx-1].1 - points[idx].1)as i32).abs() < 2 {
                    //nothing todo
                } else if points[idx-1].0 == points[idx].0 {
                    points[idx].1+= if points[idx-1].1 < points[idx].1 {-1} else {1};
                } else if points[idx].1 == points[idx-1].1 {
                    points[idx].0+= if points[idx-1].0 < points[idx].0 {-1} else {1};
                } else {
                    points[idx].0+= if points[idx-1].0 < points[idx].0 {-1} else {1};
                    points[idx].1+= if points[idx-1].1 < points[idx].1 {-1} else {1};
                }
